...
class manager():
	...
	def _formatRepoLine(self,repoline,jsonF=""):
		data={}
		...
		if len(repoUrl)>0:
			...
			if len(repoRelease)>0:
				...
				data[repoUrl][repoRelease]={"components":repoComponents,"file":jsonF,"raw":repoline,"name":os.path.basename(jsonF),"desc":""}
		return(data)
	#def _formatRepoLine
	...
	def _writeJsonFromSources(self,file,content):
		...
		if len(content)>0:
			...
			if len(jcontent)>0:
				...
				name=data[url][release].get("name",os.path.basename(file)).replace(".json",".list")
	...
	def getRepos(self):
		...
		extraRepos.update(self._readSourcesDir(self.sourcesDir))
